...
try:
    from src.models.modules.attention.blocksparse_matmul import matmul
except ImportError:
    logger.info('triton is not installed')
    matmul = None
from src.models.modules.layers.fastlinear import LowRank, FastLinear
from src.ops.butterfly_factor import butterfly_factor_to_matrix
from src.models.modules.attention.blocksparse_utils import densify_tensor
...
class BlockSparseLinear(nn.Module):
    ...
    def reset_parameters(self) -> None:
        ...
        # Scale by sqrt bc the weight is sparse
        scaling = math.sqrt(self.in_features_extended * self.out_features_extended
                            / (self.nnz_blocks * self.block_size ** 2))
        bound *= scaling
        with torch.no_grad():
    ...
    def forward(self, x):
        ...
        if self.backend == 'huggingface':
            ...
            weight = rearrange(
                densify_tensor(rearrange(self.weight, 'z b b1 -> 1 z b b1'),
                               rearrange(self.layout, 'p r -> 1 p r')),
                '1 1 m n -> m n'
            )
